...
static I2CKomunikation_PI_TDeviceData DeviceDataPrv__DEFAULT_RTOS_ALLOC;
/* {Default RTOS Adapter} Global variable used for passing a parameter into ISR */
static I2CKomunikation_PI_TDeviceDataPtr INT_I2C0__DEFAULT_RTOS_ISRPARAM;
...
PE_ISR(I2CKomunikation_PI_Interrupt)
{
  ...
  I2CKomunikation_PI_TDeviceDataPtr DeviceDataPrv = INT_I2C0__DEFAULT_RTOS_ISRPARAM;
  ...
  if (I2C_PDD_GetMasterMode(I2C0_BASE_PTR) == I2C_PDD_MASTER_MODE) { /* Is device in master mode? */
    ...
    if ((Status & I2C_PDD_ADDRESSED_AS_SLAVE) != 0x00U) { /* Addressed as a slave? */
      ...
      if ((Status & I2C_PDD_SLAVE_TRANSMIT) != 0x00U) { /* Read? */
        ...
        if (DeviceDataPrv->OutLenS < DeviceDataPrv->OutLenSReq) { /* Is the requested number of characters transmitted? */
          ...
          I2C_PDD_WriteDataReg(I2C0_BASE_PTR, I2CKomunikation_PI_EMPTY_CHAR); /* Store the empty char to the transmit register */
      ...
      if ((Status & I2C_PDD_ARBIT_LOST) != 0x00U) { /* Arbitration lost? */
        ...
        if (I2C_PDD_GetTransmitMode(I2C0_BASE_PTR) == I2C_PDD_TX_DIRECTION) { /* Transmit? */
          if ((Status & I2C_PDD_RX_ACKNOWLEDGE) != 0x00U) { /* NACK from receiver */
            ...
            (void)I2C_PDD_ReadDataReg(I2C0_BASE_PTR); /* Dummy read character */
            if (DeviceDataPrv->OutLenS < DeviceDataPrv->OutLenSReq) { /* Is the requested number of characters transmitted? */
              ErrorMask |= LDD_I2C_SLAVE_NACK; /* Set the Slave not acknowledge error mask */
            }
          } else {
            if (DeviceDataPrv->OutLenS < DeviceDataPrv->OutLenSReq) { /* Is the requested number of characters transmitted? */
          ...
          if (DeviceDataPrv->InpLenS < DeviceDataPrv->InpLenSReq) { /* Is the requested number of characters received? */
            ...
            *(DeviceDataPrv->InpPtrS)++ = I2C_PDD_ReadDataReg(I2C0_BASE_PTR); /* Save received char to the receive buffer */
            if (DeviceDataPrv->AckType == LDD_I2C_NACK_BYTE) {
              I2C_PDD_EnableTransmitAcknowledge(I2C0_BASE_PTR, PDD_DISABLE); /* Transmit NACK */
              DeviceDataPrv->InpLenSReq = 0x00U;
    ...
    if ((Flags & ON_SLAVE_BLOCK_SENT) != 0x00U) { /* Is OnSlaveBlockSent flag set? */
      I2CKomunikation_PI_OnSlaveBlockSent(DeviceDataPrv->UserData); /* If yes then invoke user event */
    }
    if ((Flags & ON_SLAVE_BLOCK_RECEIVED) != 0x00U) { /* Is OnSlaveBlockReceived flag set? */
      I2CKomunikation_PI_OnSlaveBlockReceived(DeviceDataPrv->UserData); /* If yes then invoke user event */
    }
    if (ErrorMask != 0x00U) {          /* Is any error mask set? */
      DeviceDataPrv->ErrorMask |= ErrorMask; /* Update list of error mask value */
      I2CKomunikation_PI_OnError(DeviceDataPrv->UserData); /* If yes then invoke user event */
    }
  }
}
...
LDD_TDeviceData* I2CKomunikation_PI_Init(LDD_TUserData *UserDataPtr)
{
  ...
  I2CKomunikation_PI_TDeviceData *DeviceDataPrv;
...
void I2CKomunikation_PI_Deinit(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  PE_LDD_UnregisterDeviceStructure(PE_LDD_COMPONENT_I2CKomunikation_PI_ID);
...
{
  I2CKomunikation_PI_TDeviceData *DeviceDataPrv = (I2CKomunikation_PI_TDeviceData *)DeviceDataPtr;
